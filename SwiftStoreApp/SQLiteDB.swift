...
//		println("SQLiteDB - Initialize column with type: \(type), value: \(value)")
	...
	func asInt()->Int {
		switch (type) {
			...
			case SQLITE_BLOB:
				let str = NSString(data:value as! NSData, encoding:NSUTF8StringEncoding) as NSString!
            return str.integerValue
			case SQLITE_NULL:
				return 0
			case SQLITE_DATE:
...
//		println("SQLiteDB - Init method")
		...
		if !(fm.fileExistsAtPath(path)) {
            ...
            let from = NSBundle.mainBundle().resourcePath?.stringByAppendingString("/\(dbName)")
	...
	private func prepare(sql:String, params:[AnyObject]?)->COpaquePointer {
		...
		if params != nil {
            ...
            typealias cfnc = @convention(c) (UnsafeMutablePointer<()>) -> Void
            let transient = unsafeBitCast(tranPointer, cfnc.self)
